...
const displayPrice = p => p.price == null ? (p.priceLabel || 'Price on request') : money(p.price);
const getProduct = id => products.find(p => p.id === id);
const imageSrc = url => url && /^https?:\/\//i.test(url) ? `https://images.weserv.nl/?url=${encodeURIComponent(url)}` : url;
