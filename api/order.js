...
async function sendWithFormSubmit(c,order,items){
 ...
 if(!r.ok||mail.success===false){console.error('FormSubmit notification failed',r.status,mail);return false;}
 return true;
}
module.exports=async function handler(req,res){
 ...
 try{
  ...
  let notified=false;
  try{notified=await sendWithFormSubmit(c,order,items);}catch(e){console.error('FormSubmit notification error',e);}
  if(notified)console.log(`Order notification sent via FormSubmit for order ${order.id}`);else console.error(`Order notification failed for order ${order.id}`);
  return json(res,200,{success:true,warning:notified?undefined:'Order saved, but the notification email failed.',order:{id:order.id,items,subtotal:Number(order.subtotal),delivery:Number(order.delivery),total:Number(order.total),payment:'Cash on Delivery'}});
