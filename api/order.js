...
module.exports=async function handler(req,res){
 ...
 try{
  ...
  if(!items.length)return json(res,502,{success:false,error:'Order was saved but the confirmation data was incomplete. Please contact TEEB.'});
  ...
  return json(res,200,{success:true,order:{id:order.id,items,subtotal:Number(order.subtotal),delivery:Number(order.delivery),total:Number(order.total),payment:'Cash on Delivery'}});
