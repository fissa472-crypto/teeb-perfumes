...
async function getTelegramChatId(){if(!TELEGRAM_BOT_TOKEN)return '';const r=await fetch(SUPABASE_URL+'/rest/v1/notification_config?id=eq.primary&select=telegram_chat_id',{headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+SUPABASE_KEY}});const rows=await r.json().catch(()=>[]);return r.ok&&rows?.[0]?.telegram_chat_id?String(rows[0].telegram_chat_id):'';}
async function sendWithTelegram(c,order,items){if(!TELEGRAM_BOT_TOKEN)return false;const chatId=await getTelegramChatId();if(!chatId)return false;const api='https://api.telegram.org/bot'+TELEGRAM_BOT_TOKEN+'/sendMessage';const r=await fetch(api,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:chatId,text:orderText(c,order,items)})});if(!r.ok){console.error('Telegram notification failed',r.status,await r.text().catch(()=>''));return false;}return true;}
module.exports=async function handler(req,res){
 ...
 try{
  ...
  const order=data;const items=Array.isArray(order?.items)?order.items:[];
  ...
  let telegram=false,email=false;
  try{telegram=await sendWithTelegram(c,order,items);}catch(e){console.error('Telegram notification error',e);}
  try{email=await sendWithFormSubmit(c,order,items);}catch(e){console.error('FormSubmit notification error',e);}
  if(telegram)console.log(`Telegram notification sent for order ${order.id}`);else console.error(`Telegram notification failed for order ${order.id}`);
  if(email)console.log(`Email notification sent for order ${order.id}`);else console.error(`Email notification failed for order ${order.id}`);
  const warning=telegram&&email?undefined:(!telegram&&!email?'Order saved, but Telegram and email notifications failed.':(!telegram?'Order saved, but Telegram notification failed.':'Order saved, but email notification failed.'));
  return json(res,200,{success:true,warning,order:{id:order.id,items,subtotal:Number(order.subtotal),delivery:Number(order.delivery),total:Number(order.total),payment:'Cash on Delivery'}});
